...
static int l_runtime_getWorldTransform(lua_State* L) {
    ...
    lua_newtable(L);
    lua_pushnumber(L, wx);    lua_setfield(L, -2, "x");
    lua_pushnumber(L, wy);    lua_setfield(L, -2, "y");
    lua_pushnumber(L, wAngle); lua_setfield(L, -2, "angle");
    lua_pushnumber(L, wsx);   lua_setfield(L, -2, "sx");
    lua_pushnumber(L, wsy);   lua_setfield(L, -2, "sy");
    return 1;
}
